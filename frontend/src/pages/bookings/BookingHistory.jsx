...
import ReviewModal from "../../components/tours/ReviewModal";
...
const BookingHistory = () => {
  ...
  const [reviewModal, setReviewModal] = useState({ open: false, tourId: null, bookingId: null, reviewData: null });
  ...
  const fetchUserReviews = async () => {
    try {
      const res = await axios.get(`${backendUrl}/api/reviews/user`, {
      ...
      if (res.data) {
        ...
        res.data.forEach((rev) => {
          if (rev.bookingId) {
            reviewsMap[rev.bookingId.toString()] = rev;
          }
        });
  ...
  const fetchBookings = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const completed = response.data.bookings.filter((b) => b.status === "completed");
                      ...
                      reviews[booking._id] ? (
                        <button
                          onClick={() => setReviewModal({
                            ...
                            reviewData: reviews[booking._id]
                          })}
                          className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-stone-200 text-stone-600 font-medium hover:bg-stone-50 hover:text-stone-900 transition-all active:scale-95 dark:border-stone-700 dark:text-stone-300 dark:hover:bg-stone-800 dark:hover:text-white"
                        >
                          Edit Review
                        </button>
                      ) : (
                        <button
                          ...
                          className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-accent-700 text-white font-medium hover:bg-accent-800 transition-all active:scale-95 shadow-sm shadow-accent-700/10"
                        >
                          Rate & Review
                        </button>
                      )
      ...
      <AnimatePresence>
        {reviewModal.open && (
          <ReviewModal
            ...
            onClose={() => setReviewModal({ open: false, tourId: null, bookingId: null, reviewData: null })}
            tourId={reviewModal.tourId}
            bookingId={reviewModal.bookingId}
            reviewData={reviewModal.reviewData}
